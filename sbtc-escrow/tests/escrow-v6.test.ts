...
describe("sBTC Escrow V6 — Dual Token Contract Tests", () => {
  ...
  describe("Admin Functions", () => {
    ...
    describe("set-dispute-timeout", () => {
      it("admin can update dispute timeout", () => {
        const { result } = simnet.callPublicFn(
          CONTRACT, "set-dispute-timeout", [Cl.uint(144)], deployer
        );
        expect(result).toBeOk(Cl.bool(true));
      });
      ...
      it("non-owner cannot update dispute timeout", () => {
        const { result } = simnet.callPublicFn(
          CONTRACT, "set-dispute-timeout", [Cl.uint(144)], buyer
        );
        expect(result).toBeErr(Cl.uint(ERR_UNAUTHORIZED));
      });
      ...
      it("rejects timeout below safe minimum (143)", () => {
        const { result } = simnet.callPublicFn(
          CONTRACT, "set-dispute-timeout", [Cl.uint(143)], deployer
        );
        expect(result).toBeErr(Cl.uint(2011));
      });
  ...
  describe("resolve-expired-dispute respects updated timeout", () => {
    it("works at the minimum timeout", () => {
      simnet.callPublicFn(CONTRACT, "set-dispute-timeout", [Cl.uint(144)], deployer);
      createStxEscrow();
      simnet.callPublicFn(CONTRACT, "dispute", [Cl.uint(1)], buyer);

      simnet.mineEmptyBlocks(140);
