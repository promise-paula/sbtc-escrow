;; title: sBTC Escrow Mainnet V2
;; version: 2.0.0
;; summary: Mainnet release of the v7-equivalent Dual-Token Escrow with on-chain
;;          delivery signal and partial dispute splits. Functionally identical to
;;          `escrow-v7` on testnet -- deployed under the mainnet-v2 name to make
;;          the contract version visible in the principal.
;; description: Successor to `escrow-mainnet` (v6-equivalent). Adds:
;;              - STATUS_DELIVERED + deliver() so a seller can signal completion on-chain
;;              - REVIEW_PERIOD that blocks buyer-initiated refund immediately after delivery
;;              - Tightened refund authorization (no more "anyone after expiry")
;;              - resolve-dispute-split for partial admin/arbiter resolutions
;;
;; Token types:
...
;; - State updates precede fund transfers (CEI ordering) on every
;;   release / refund / dispute resolution to defeat reentrancy.
;; - MIN_DISPUTE_TIMEOUT enforces a ~3.5h floor so an admin cannot
;;   shorten the window enough to let buyers self-recover before review.
;; - REVIEW_PERIOD (144 blocks, ~3.5h) after deliver() blocks buyer
;;   refund. Seller has this window to raise a dispute if the buyer
;;   is stalling acceptance after work was delivered.
;; - refund() after expiry is buyer-only (was: anyone). Removes
...
(define-constant MIN_DISPUTE_TIMEOUT u144)     ;; ~3.5h minimum -- protects against admin
                                                ;; setting a window so short the buyer can
                                                ;; self-recover before any arbiter review.
...
;; Post-delivery review window. While inside this window after deliver(),
;; the buyer cannot unilaterally refund -- only release or dispute.
;; Gives the seller a guaranteed opportunity to dispute if the buyer is
;; trying to stall acceptance after work was delivered.
(define-constant REVIEW_PERIOD u144)           ;; ~3.5h at 960 blocks/day
...
;; Returns true if the escrow's post-delivery review window has fully elapsed,
...
(define-private (is-review-period-elapsed (delivered-at-opt (optional uint)))
  (match delivered-at-opt
    da (> stacks-block-height (+ da REVIEW_PERIOD))
    true
  )
)
...
;;
;; Moves the escrow into STATUS_DELIVERED and records delivered-at. While the
;; review window (REVIEW_PERIOD blocks) is active the buyer cannot unilaterally
;; refund -- they can only release or dispute. This guarantees the seller a
;; minimum window in which to raise a dispute if the buyer is stalling
...
;;
;; Buyer can ALWAYS release to seller, from PENDING or DELIVERED, even after
;; expiry. Reads token-type from stored escrow to determine transfer method.
(define-public (release (escrow-id uint))
  ...
  (let (
    ...
    ;; Must be pending or delivered
    (asserts!
      (or (is-eq status STATUS_PENDING) (is-eq status STATUS_DELIVERED))
      ERR_ESCROW_ALREADY_COMPLETED
    )

    ;; Update escrow status FIRST (CEI: prevents reentrancy via token transfer hooks)
    (map-set escrows escrow-id (merge escrow {
...
;;
;; Authorization:
;; - Seller may refund voluntarily at any time, from PENDING or DELIVERED.
;;   (The seller giving up their claim is always allowed.)
;; - Buyer may refund only after expires-at AND after any review period
;;   triggered by deliver() has fully elapsed. This closes the window where a
;;   buyer could wait out expires-at to grab a refund of already-delivered
;;   work without the seller having a chance to dispute.
;; - Anyone-after-expiry is no longer permitted. (Was a gap in v6.)
(define-public (refund (escrow-id uint))
  ...
  (let (
    ...
    (asserts!
      (or
        ;; Seller voluntarily refunds (anytime, either status)
        (is-eq contract-caller seller)
        ;; Buyer refunds after expiry AND after any review window
        (and
          (is-eq contract-caller buyer)
...
;; Raise a dispute (buyer or seller, from PENDING or DELIVERED)
...
;; Resolve dispute with a partial split (admin only).
;;
;; buyer-bps is the buyer's share of the principal in basis points (0-10000).
;; The seller receives (BPS_DENOMINATOR - buyer-bps) of the principal. The
;; original fee is split pro-rata: the buyer is refunded the fee on the
;; portion they got back (their share); the platform keeps the fee on the
;; portion that went to the seller.
;;
;; Bookkeeping:
;; - total-released is incremented (this resolution settled the escrow).
;; - add-fees credits only the platform's pro-rata fee share.
;; - Seller stats credit the seller's share against received totals.
;;
;; Extremes:
;; - buyer-bps = 0       => identical effect to resolve-dispute-for-seller.
;; - buyer-bps = 10000   => identical effect to resolve-dispute-for-buyer
;;   (except final status is STATUS_RELEASED rather than STATUS_REFUNDED;
;;   prefer the dedicated functions when settling at the extremes for
;;   cleaner accounting and event semantics).
(define-public (resolve-dispute-split (escrow-id uint) (buyer-bps uint))
  ...
  (let (
    ...
    ;; Pay platform its pro-rata fee (if any)
    (if (> platform-fee u0)
      (if (is-eq token-type TOKEN_STX)
        (try! (stx-withdraw platform-fee (var-get fee-recipient)))
        (try! (sbtc-withdraw platform-fee (var-get fee-recipient)))
      )
      true
    )
    ...
    ;; Credit seller stats for their share
    (if (> seller-share u0)
      (map-set user-stats seller (merge seller-stats {
        escrows-received: (+ (get escrows-received seller-stats) u1),
        total-received-stx: (if (is-eq token-type TOKEN_STX)
...
;; True iff the escrow has been delivered and is still inside the review window.
