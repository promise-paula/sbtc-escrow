...
description: "Core escrow operations — create, release, refund, dispute, extend"
---
...
<ParamField path="seller" type="principal" required>
  The recipient's Stacks address. Cannot be the same as `tx-sender`.
</ParamField>

<ParamField path="amount" type="uint" required>
  The escrow amount in smallest unit (microSTX for STX, satoshis for sBTC). Does **not** include the fee — the contract calculates and adds it.
</ParamField>

<ParamField path="description" type="string-utf8 256" required>
  A human-readable description of the escrow purpose. Max 256 UTF-8 characters.
</ParamField>

<ParamField path="duration" type="uint" required>
  Duration in Stacks blocks until the escrow expires. Must be between 1 and 350,400 (~365 days).
</ParamField>

<ParamField path="token-type" type="uint" required>
  `u0` for STX, `u1` for sBTC.
</ParamField>

### Returns

<ResponseField name="ok" type="uint">
  The newly created escrow ID (auto-incrementing).
</ResponseField>
...
<ParamField path="escrow-id" type="uint" required>
  The ID of the escrow to release.
</ParamField>

### Authorization

**Buyer only.** The caller must be the escrow's `buyer`.
...
<Info>
Release works **even after expiry**. The buyer can always choose to release regardless of the deadline.
</Info>
...
<ParamField path="escrow-id" type="uint" required>
  The ID of the escrow to refund.
</ParamField>
...
<ParamField path="escrow-id" type="uint" required>
  The ID of the escrow to dispute.
</ParamField>

### Authorization

**Buyer or seller.** Either party in the escrow can raise a dispute.
...
<Note>
Disputes can be raised **after expiry**. This allows parties to dispute even if the deadline has passed, preventing a race condition where funds are refunded just before a dispute is filed.
</Note>
...
<ParamField path="escrow-id" type="uint" required>
  The ID of the escrow to extend.
</ParamField>

<ParamField path="additional-blocks" type="uint" required>
  Number of additional blocks to add to the deadline. Must result in a total remaining duration ≤ MAX_DURATION.
</ParamField>

### Authorization

**Buyer only.** Must be called before the escrow expires.
